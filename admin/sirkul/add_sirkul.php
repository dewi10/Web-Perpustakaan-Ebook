					...
					<div class="box-body">
						...
						<div class="form-group">
							...
							<select name="id_buku" id="id_buku" class="form-control select2" style="width: 100%;">
								...
								<?php
								// ambil data dari database
								$query = "select * from tb_buku where jumlah > 0 order by seri_buku, judul_buku";
								$hasil = mysqli_query($koneksi, $query);
								while ($row = mysqli_fetch_array($hasil)) {
								?>
								<option value="<?php echo $row['id_buku'] ?>">
									...
									-
									<?php echo htmlspecialchars(format_judul_buku($row['seri_buku'], $row['judul_buku'])); ?>
									(<?php echo $row['jumlah'] ?>)
								</option>
								<?php
								}
...
<?php
    ...
    if (isset ($_POST['Simpan'])){
        ...
        $stok_cek = mysqli_query($koneksi, "SELECT jumlah FROM tb_buku WHERE id_buku='".$_POST['id_buku']."'");
        $stok_data = mysqli_fetch_assoc($stok_cek);
        $stok = isset($stok_data['jumlah']) ? (int) $stok_data['jumlah'] : 0;

        if ($stok < 1) {
            echo "<script>
            Swal.fire({title: 'Stok buku habis',text: '',icon: 'error',confirmButtonText: 'OK'
            }).then((result) => {
                if (result.value) {
		...
		$sql_simpan .= "INSERT INTO log_pinjam (id_buku,id_anggota,tgl_pinjam) VALUES (
			...
			'".$_POST['id_anggota']."',
            '".$_POST['tgl_pinjam']."');";
        $sql_simpan .= "UPDATE tb_buku SET jumlah = jumlah - 1 WHERE id_buku='".$_POST['id_buku']."' AND jumlah > 0";
